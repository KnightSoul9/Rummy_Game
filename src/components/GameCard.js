...
const GameCard = memo(({ id, name, image }) => {
  const router = useRouter();

  // Generate random users only ONCE when component mounts
  const users = useMemo(() => {
    const min = 100000;
    const max = 700000000;
    const randomUsers = Math.floor(Math.random() * (max - min + 1)) + min;
    return randomUsers >= 1000000
      ? `${(randomUsers / 1000000).toFixed(1)}M+`
      : `${(randomUsers / 1000).toFixed(1)}K+`;
  }, []);
  ...
  return (
    <div
      ...
      className="relative bg-gradient-to-br from-gray-100/90 via-gray-50/80 to-gray-100/90 p-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 w-full overflow-hidden group cursor-pointer"
    >
      ...
      <div className="relative aspect-[4/3] w-full">
        ...
        />
        {/* Users Count */}
        <div className="absolute top-2 right-2 bg-gray-800/70 backdrop-blur-sm rounded-full px-2 py-1 text-xs text-gray-200 flex items-center">
          <svg
      ...
      {/* Game Name */}
      <h3 className="text-lg font-semibold mt-2 text-center truncate bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">
        {name}
      </h3>
